...
let take_turn (Simple(lookahead)) gt =
  let my_color =
    gt |> Game_tree.get_state |> GS.get_current_player |> PS.get_player_color in
  ...
  (* Evaluate [gt] from the perspective of player with [color] without looking
   * ahead *)
  let evaluate_state (gs : Game_state.t) : int =
    PS.get_score @@ GS.get_player_with_color gs my_color in
  ...
  let rec evaluate_tree (turns_left : int) (gt : Game_tree.t) : int =
    if turns_left = 0
    then evaluate_state @@ Game_tree.get_state gt
    else 
      ...
      | None -> evaluate_state @@ Game_tree.get_state gt      
      | Some(score) -> score
  in
  let best_scored_move =
    Game_tree.get_subtrees gt
    |> List.map ~f:(fun (act, subt) -> (act, evaluate_tree (lookahead - 1) subt))
