...
let () =
  let players = List.init 4 ~f:(fun i ->
      Player.create_AI_player ~name:(string_of_int i)
        Player.Strategy.Penguin_placer.create_scanning_strategy
        (Player.Strategy.Turn_actor.create_minimax_strategy i)) in
  let config = Config.create ~height:5 ~width:5
  ...
  Core.ignore @@ Referee.run_game referee players config;
